...
export type ExtConfig = {
  ...
  'inlayHints.showInferredTypes': boolean
  'server.extraArgs': string[]
  'server.includeDirs': string[]
  'server.invocationTimeout': number
  'server.path': string
  'server.trace': 'off' | 'messages' | 'compact' | 'verbose'
}
...
export function getExtConfig<const T extends keyof ExtConfig>(
  ...
  return workspace.getConfiguration('nushell', scope).get<ExtConfig[T]>(key)!
}
